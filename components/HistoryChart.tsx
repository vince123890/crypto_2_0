    ...
    <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
      <div className="mb-2 flex items-center justify-between border-b border-white/10 pb-2">
        <h3 className="text-sm font-medium uppercase tracking-wider text-neutral-300">
          Histori CLSS vs Harga ({symbol})
        </h3>
        <button
          onClick={() => downloadHistoryCSV(symbol)}
          disabled={data.length === 0}
          className="rounded-md border border-white/15 px-3 py-1 text-xs uppercase tracking-wider text-neutral-300 transition hover:border-white/30 hover:bg-white/5 disabled:opacity-40"
        >
          Export CSV
        </button>
      </div>
      ...
      {data.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={data}>
            ...
            <YAxis yAxisId="left" domain={[-100, 100]} stroke="#737373" fontSize={11} />
            <YAxis yAxisId="right" orientation="right" stroke="#737373" fontSize={11} />
            <Tooltip contentStyle={{ background: '#000000', border: '1px solid #262626', fontSize: 12 }} />
